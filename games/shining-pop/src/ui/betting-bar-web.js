/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   v8 port of the delivered web betting bar (betting_bar_web spec). Design 2400x300.
   Layout: ACCOUNT(menu+sound+BALANCE) | LAST WIN | TOTAL BET | swipe bet
           carousel | coins(buy) | ×2 | turbo/autoplay | SPIN.

   Rendered with v8 FillGradient (VECTOR) — crisp borders at any scale, no
   masked-sprite blur. Crystal palette (harmonises with the candy bg).

   API: setBalance/setBet/setLastWin/setCurrency/setSoundOn/setBetLevels +
        setSpinning/setAutoplay/setTurbo/setAffordable.
   Events (on2): spin · bet:set(absoluteIdx) · bet:double · autoplay · turbo · sound · menu · buy
   fitBottom(W,H) docks to the screen bottom (width-fit).
   ============================================================================ */
...
export class BettingBarWeb extends PIXI.Container {
  ...
  _build() {
    ...
    // ACCOUNT PANEL (440 wide — slimmer so the info row isn't ~60% of the bar)
    ...
    // ── BET CAROUSEL — swipeable quick-bets through ALL levels, center=active ──
    const SX = 1100, SW = 800, CELLW = 132, PCX = SW / 2; // selector x, width, cell pitch, local center
    ...
    // ── RIGHT CLUSTER — breathing gaps, SPIN flush-right ──
  ...
  // Populate the swipe carousel with ALL bet levels + center the active one.
