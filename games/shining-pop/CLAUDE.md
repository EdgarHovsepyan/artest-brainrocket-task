...
- **Never measure a masked container whose mask lives OUTSIDE it.** Reading `.height`/`getBounds()` of a body whose mask is parented to the surrounding card makes Pixi `warn("Mask bounds, renderable is not inside the root container")` — in PRODUCTION, ungateable by `?debug` → Stake §4 reject. Detach → measure → reattach (`_refreshScroll` and `openDrawer` both do this now). (2026-06-10, fixed `f80dd98`.)
- **`modalSnap` must re-capture the rest pose, not restore `_modalBaseY`.** The stored baseY goes stale when the viewport changes while a modal is open (rotation / preset switch): restoring desktop y≈338 onto Popout S H=225 parks the panel fully below the screen. The caller re-lays-out right before snapping, so snap to the card's CURRENT y and honor `_fitScale`. (2026-06-10, fixed `f80dd98`.)
